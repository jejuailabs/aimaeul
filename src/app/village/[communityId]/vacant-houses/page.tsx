...
import { formatRent, relativeTime, communityTypeMeta } from '@/lib/village'
import { ThemeToggle } from '@/components/theme-toggle'
import { Button } from '@/components/ui/button'
import { CommunityBadge } from '@/components/community-badge'

export const dynamic = 'force-static'
export const revalidate = 300 // 5분 ISR — 검색엔진 친화적
...
async function getData(communityId: string) {
  ...
  if (!community) return { community: null, listings: [] }
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if (!community) {
    return { title: '빈집 목록 — 우리마을' }
  }
  const meta = communityTypeMeta(community.communityType)
  return {
    title: `${community.name} 빈집 — 우리마을`,
    description: `${community.regionName} ${community.name}(${meta.label}) 빈집 임대 정보. 보증금/월세와 사진으로 확인하세요.`,
    keywords: [
      '빈집',
      '임대',
      '전세',
      ...
      <div className="flex min-h-screen flex-col bg-background">
        <header className="sticky top-0 z-30 flex items-center gap-2 border-b border-border/60 bg-background/95 px-2 py-2 backdrop-blur">
          <Button asChild variant="ghost" size="icon" className="rounded-full">
            <Link href={`/village/${communityId}`} aria-label="마을 홈으로">
        ...
        {listings.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-muted/30 px-4 py-16 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              마을 회원이 새로운 빈집을 등록하면 여기에 표시됩니다.
            </p>
            <Button asChild className="mt-5 rounded-full">
              <Link href={`/login?callbackUrl=/app/vacant-houses`}>
                로그인하고 빈집 등록하기
              </Link>
            </Button>
          </div>
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-1.5 p-4">
                    <p className="text-base font-bold">
                      {formatRent(l.monthlyRent, l.deposit)}
                    </p>
                    <p className="text-[11px] text-muted-foreground">
                      <MapPin className="mr-0.5 inline h-3 w-3" />
                      {community.regionName} · {community.name}
                    </p>
                    {l.description && (
                    ...
                    >
                      <Link
                        href={`/login?callbackUrl=/app/chat/${communityId}`}
                      >
