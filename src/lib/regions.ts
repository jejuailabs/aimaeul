/**
 * 가입 시 마을을 찾기 위한 행정구역 데이터 (시도 → 시군구 2단계).
 *
 * 고령 사용자를 배려해 단계는 2개로 제한한다. 읍면동은 `communities.regionName`에
 * 자유 문자열로 남아 있으므로 표시용으로만 쓴다.
 */
...
/** `regionName`(예: "경상북도 영양군 석보면")에서 시도/시군구를 추출한다. */
export function parseRegion(regionName: string): { sido: string | null; sigungu: string | null } {
  const parts = (regionName || '').trim().split(/\s+/)
  const sido = SIDO_LIST.find((s) => parts[0] === s) ?? null
  if (!sido) return { sido: null, sigungu: null }
  const candidates = SIGUNGU_BY_SIDO[sido] ?? []
  const sigungu = candidates.find((g) => parts.includes(g)) ?? parts[1] ?? null
  return { sido, sigungu }
}

export const COMMUNITY_TYPES = ['부녀회', '청년회', '노인회', '동호회'] as const
